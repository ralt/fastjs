...
function parse(data, fullpath, basepath, puts) {
    ...
    if (isFirst(putsEndIdx, indexes)) {
        ...
        ret += currentModule[currentModule.length - 1]
            .puts[currentModule[currentModule.length - 1].puts.length - 1]
            .value;
    ...
    if (isFirst(includesEndIdx, indexes)) {
        ...
        var lastModule = currentModule.pop();
        if (currentModule[currentModule.length - 1] &&
                currentModule[currentModule.length - 1].puts.length) {
            currentModule[currentModule.length - 1]
                .puts[currentModule[currentModule.length - 1].puts.length - 1]
                .value += parse(content, fullpath, basepath, lastModule.puts);
        }
        else {
            ret += parse(content, fullpath, basepath, lastModule.puts);
        }

        currentModule.forEach(function(m) {
            console.log(m);
        });
