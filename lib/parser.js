...
function parsePage(page, content, fullpath, basepath, idx) {
    ...
    if (isFirst(includesBeginIdx, indexes)) {
        ...
        page.modules.push(parsePage(newPage, content.slice(includesBeginIdx + includesBeginMatch[0].length), fullpath, basepath, newPage.beginIdx));

        return parsePage(page, content.slice(page.modules.last().endIdx), fullpath, basepath);
    }
