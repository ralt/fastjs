...
function parse(data, fullpath, basepath, puts) {
    ...
    if (isFirst(includesBeginIdx, indexes)) {
        ...
        return parse(data.slice(includesBeginIdx + includesBeginMatch[0].length), fullpath, basepath);
    }
    ...
    if (isFirst(putsBeginIdx, indexes)) {
        ...
        return parse(data.slice(putsBeginIdx + putsBeginMatch[0].length), fullpath, basepath);
    }
    ...
    if (isFirst(putsEndIdx, indexes)) {
        ...
        return parse(data.slice(putsEndIdx + putsEndMatch[0].length), fullpath, basepath);
    }
