...
function parsePage(page, content, fullpath, basepath, idx) {
    ...
    if (isFirst(includesBeginIdx, indexes)) {
        ...
        parsePage(newPage, content.slice(includesBeginIdx + includesBeginMatch[0].length), fullpath, basepath, newPage.beginIdx);

        return parsePage(page, content.slice(page.modules.last().endIdx), fullpath, basepath, idx + includesBeginIdx + includesBeginMatch[0].length);
    }
