...
function parse(ret, data, fullpath, basepath, puts) {
    ...
    if (isFirst(includesBeginIdx, indexes)) {
        ...
        return parse(ret, data.slice(includesBeginIdx + includesBeginMatch[0].length), fullpath, basepath);
    }
    ...
    if (isFirst(putsBeginIdx, indexes)) {
        ...
        return parse(ret, data.slice(putsBeginIdx + putsBeginMatch[0].length), fullpath, basepath);
    }
    ...
    if (isFirst(putsEndIdx, indexes)) {
        ...
        return parse(ret, data.slice(putsEndIdx + putsEndMatch[0].length), fullpath, basepath);
    }
    ...
    if (isFirst(paramBeginIdx, indexes)) {
        ...
        return parse(ret, data.slice(paramBeginIdx + paramBeginMatch[0].length), fullpath, basepath, puts);
    }
    ...
    if (isFirst(paramEndIdx, indexes)) {
        ...
        return parse(ret, data.slice(paramEndIdx + paramEndMatch[0].length, fullpath, basepath, puts));
    }
